...
use crate::{crc16::{self, update_crc32}, VERSION};
...
{
    let crc = crc16::get_crc32(block);
    format!("{:08X}", crc)
}

pub fn get_crc16string(block: &[u8]) -> String
{
    let crc = crc16::get_crc16(block);
    format!("{:04X}", crc)
}
...
fn parse_emsi_blocks(data: &[u8]) -> io::Result<Vec<String>>
{
    ...
    while i < data.len() {
        ...
        if data[i] == b'\\' {
            ...
            if i + 2 < data.len() {
                let b = get_value(data[i + 1]) * 16 + get_value(data[i + 2]);
                str.push(char::from_u32(b as u32).unwrap());
                i += 3;
                continue;
            }
