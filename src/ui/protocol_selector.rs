...
pub fn view_protocol_selector<'a>(download: bool) -> Element<'a, Message> {
    ...
    column![
        ...
        row![ horizontal_space(Length::Units(left)), button("Xmodem 1k").on_press(Message::SelectProtocol(ProtocolType::XModem1k, download)).width(Length::Units(button_width)), horizontal_space(Length::Units(space)), text("Rarely used anymore")],
        row![ horizontal_space(Length::Units(left)), button("Xmodem 1k-G").on_press(Message::SelectProtocol(ProtocolType::XModem1kG, download)).width(Length::Units(button_width)), horizontal_space(Length::Units(space)), text("Does that even exist?")],
        ...
        row![ horizontal_space(Length::Units(left)), button("Ymodem-G").on_press(Message::SelectProtocol(ProtocolType::YModemG, download)).width(Length::Units(button_width)), horizontal_space(Length::Units(space)), text("A fast Ymodem variant")]
    ].spacing(8).into()
}
