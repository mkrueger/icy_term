...
impl<'a> canvas::Program<Message> for BufferView {
    ...
    fn draw(
        ...
        let first_line = self.buf.layers[0].lines.len() as i32 - self.buf.height;
        ...
        let content =
                ...
                for y in 0..self.buf.height as usize {
                    for x in 0..self.buf.width as usize {
                        let rect  = Rectangle::new(
                            ...
                            if let Some(ch) = buffer.get_char(Position::from(x as i32, first_line + y as i32)) {
                                let (fg, bg) = 
            ...
            if !self.blink {
                ...
                let p = Point::new(
                    ...
                    top_y + ((self.caret.get_position().y - first_line) as f32 * char_size.height) as f32 + 0.5 + char_size.height - caret_size.height);
