...
pub unsafe fn run_sim(window: &mut MainWindow) {
    ...
    for offset in CUR_OFFSET..upper {
        if let Err(err) = window.buffer_view.buffer_parser.print_char(&mut window.buffer_view.buf, &mut window.buffer_view.caret, char::from_u32_unchecked(TXT[offset] as u32)) {
            eprintln!("{}", err);
        }
