...
impl std::fmt::Display for TransmissionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TransmissionError::ZLDEExected(b) => write!(f, "ZDLE expected got {} (0x{:X})", char::from_u32(*b as u32).unwrap(), b),
            TransmissionError::UnknownHeaderType(ht) => write!(f, "unknown header type {}", ht),
            TransmissionError::CRC16Mismatch(crc, check_crc) => write!(f, "crc16 mismatch got {:04X} expected {:04X}", crc, check_crc),
            TransmissionError::CRC32Mismatch(crc, check_crc) => write!(f, "crc32 mismatch got {:08X} expected {:08X}", crc, check_crc),
