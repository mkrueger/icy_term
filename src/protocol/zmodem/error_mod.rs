    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TransmissionError::InvalidSubpacket(m) => {
                write!(f, "don't understand subpacket {m}/x{m:X}")
            }
            ...
            TransmissionError::CRC16Mismatch(crc, check_crc) => {
                write!(f, "crc16 mismatch got {crc:04X} expected {check_crc:04X}")
            }
            TransmissionError::CRC32Mismatch(crc, check_crc) => {
                write!(f, "crc32 mismatch got {crc:08X} expected {check_crc:08X}")
            }
