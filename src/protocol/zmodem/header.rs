...
impl Display for Header {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.frame_type {
            FrameType::ZRPOS | FrameType::ZEOF | FrameType::ZFREECNT | FrameType::ZDATA => write!(
                f,
                "[{:?} Header with {:?} number = {}]",
                self.header_type,
                self.frame_type,
            ...
            FrameType::ZCRC | FrameType::ZCHALLENGE => write!(
                f,
...
mod tests {
    use crate::protocol::{
        zmodem::header::{Header, HeaderType},
        *,
    };
    ...
    fn test_hex_header_data() {
        ...
        assert_eq!(
            "**\x18B00000000000000\n\x11".to_string(),
            String::from_utf8(
                Header::from_flags(HeaderType::Hex, FrameType::ZRQINIT, 0, 0, 0, 0).build()
            )
            .unwrap()
        );
        assert_eq!(
            "**\x18B0100000000aa51\n\x11".to_string(),
            String::from_utf8(
                Header::from_flags(HeaderType::Hex, FrameType::ZRINIT, 0, 0, 0, 0).build()
            )
            .unwrap()
        );
        assert_eq!(
            "**\x18B02000000004483\n\x11".to_string(),
            String::from_utf8(
                Header::from_flags(HeaderType::Hex, FrameType::ZSINIT, 0, 0, 0, 0).build()
            )
            .unwrap()
        );
        assert_eq!(
            "**\x18B0300000000eed2\n".to_string(),
            String::from_utf8(
                Header::from_flags(HeaderType::Hex, FrameType::ZACK, 0, 0, 0, 0).build()
            )
            .unwrap()
        );
        assert_eq!(
            "**\x18B087e0400003ec2\n".to_string(),
            String::from_utf8(
                Header::from_flags(HeaderType::Hex, FrameType::ZFIN, 126, 4, 0, 0).build()
            )
            .unwrap()
        );
    }
